...
export default function HomePage() {
  const { datasets, loading, error, fetchDatasets, createDataset, deleteDataset } = useDatasetStore();
  ...
  return (
    <MainLayout>
      ...
      <div
        className={`fixed inset-0 z-50 flex items-center justify-center ${showCreateModal ? '' : 'hidden'}`}
      >
        <div className="absolute inset-0 bg-black/50" onClick={() => setShowCreateModal(false)} />
        <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 w-full max-w-md animate-scale-in">
          <h2 className="text-lg font-semibold mb-4">创建数据集</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">数据集名称</label>
              <input
                type="text"
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewDataset({ ...newDataset, taskType: e.target.value as TaskType })}
                className="w-full h-10 px-3 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"
              >
            ...
            <div>
              <label className="block text-sm font-medium mb-1">类别（每行一个）</label>
              <textarea
